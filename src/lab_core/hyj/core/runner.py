...
class BaseRunner(ABC):
    ...
    def run(
        ...
        *,
        ...
        pd.DataFrame({"target": RfLgbmRunner._to_int_pred(pred)}).to_csv(
...
class RfLgbmRunner(BaseRunner):
    ...
    def __init__(self, builder: BaseDatasetBuilder, *, params: ModelParams) -> None:
        super().__init__(builder)
        self.params = params
    ...
    def run_ensemble(
        ...
        *,
        ...
        pd.DataFrame({"target": RfLgbmRunner._to_int_pred(ensemble)}).to_csv(
            pred_path, index=False
        )
        pd.DataFrame({"target": RfLgbmRunner._to_int_pred(pred_lgbm)}).to_csv(
            pred_lgbm_path, index=False
        )
        pd.DataFrame({"target": RfLgbmRunner._to_int_pred(pred_rf)}).to_csv(
