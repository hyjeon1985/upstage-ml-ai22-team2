...
class HPPDatasetBuilder(BaseDatasetBuilder):
    ...
    FEATURE_COLS_RF = [c for c in FEATURE_COLS if c not in {"아파트명"}]
    RAW_FILES = {
        "TRAIN": "train.csv",
        "TEST": "test.csv",
    ...
    def _load_raw_datas(
        ...
        subway_path = raw_data_dir(self.RAW_FILES["SUBWAY"])
        bus_path = raw_data_dir(self.RAW_FILES["BUS"])
    ...
    def _load_ext_datas(self) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def load_raw(self) -> tuple[pd.DataFrame, pd.DataFrame, str]:
        train, test, _, _ = self._load_raw_datas()
        return train, test, "target"

    def load_ext(self) -> dict[str, Any]:
        # 여기서 한 번 로드해서 캐시 → make_pipeline에서 주입
        _, _, subway, bus = self._load_raw_datas()
        apt, coord = self._load_ext_datas()
